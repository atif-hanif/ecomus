	...
	var staggerWrap = function() {
		if($(".stagger-wrap").length) {
			...
			for(var i = 1, time = 0.2; i <= count; i++) {
				$(".stagger-item:nth-child(" + i + ")").css("transition-delay", time * i + "s").addClass("stagger-finished");
			}
		}
	};
	/* filter
	...
	var totalVariant = function() {
		$(".tf-variant-item,.tf-cart-item").each(function() {
			...
			function updateTotalPrice(price, scope) {
				...
				var totalPrice = currentPrice * quantity;
				scope.find(".tf-variant-item-total .price").text("$" + totalPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ","));
			}
		});
	};
	// Dom Ready
