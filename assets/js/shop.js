...
(function($) {
	...
	var filterProducts = function() {
		...
		function updateMetaFilter() {
			...
			if(filters.availability) {
				appliedFilters.append(`<span class="filter-tag">${filters.availability} <span class="remove-tag icon-close" data-filter="availability"></span></span>`);
			}
			if(filters.size) {
				appliedFilters.append(`<span class="filter-tag">${filters.size} <span class="remove-tag icon-close" data-filter="size"></span></span>`);
			}
			if(filters.minPrice > minPrice || filters.maxPrice < maxPrice) {
				appliedFilters.append(`<span class="filter-tag">$${filters.minPrice} - $${filters.maxPrice} <span class="remove-tag icon-close" data-filter="price"></span></span>`);
			}
			if(filters.color) {
				const colorElement = $(`input[name="color"][value='${filters.color}']`);
				const backgroundClass = colorElement.attr("class").split(" ").find((cls) => cls.startsWith("bg_"));
				const line = backgroundClass === "bg_white" ? "border-line-dark" : "";
				appliedFilters.append(`<div class="filter-tag color-tag">
			...
			if(filters.brands) {
				appliedFilters.append(`<span class="filter-tag">${filters.brands} <span class="remove-tag icon-close" data-filter="brand"></span></span>`);
			}
			if(filters.sale) {
				appliedFilters.append(`<span class="filter-tag on-sale">On Sale <span class="remove-tag icon-close" data-filter="sale"></span></span>`);
			}
	...
	var filterSort = function() {
		...
		function applyFilter(sortValue, isListActive) {
			...
			if(sortValue === "price-low-high") {
				products.sort(
					(a, b) => parseFloat($(a).find(".current-price").text().replace("$", "")) - parseFloat($(b).find(".current-price").text().replace("$", "")));
			} else if(sortValue === "price-high-low") {
				products.sort(
					(a, b) => parseFloat($(b).find(".current-price").text().replace("$", "")) - parseFloat($(a).find(".current-price").text().replace("$", "")));
	...
	var swLayoutShop = function() {
		...
		function hasValidLayout() {
			return($("#gridLayout").hasClass("tf-col-2") || $("#gridLayout").hasClass("tf-col-3") || $("#gridLayout").hasClass("tf-col-4") || $("#gridLayout").hasClass("tf-col-5") || $("#gridLayout").hasClass("tf-col-6") || $("#gridLayout").hasClass("tf-col-7"));
		}
		...
		function updateLayoutDisplay() {
			...
			if(userSelectedLayout) {
				...
				} else if(windowWidth <= 1400 && (userSelectedLayout === "tf-col-5" || userSelectedLayout === "tf-col-6" || userSelectedLayout === "tf-col-7")) {
